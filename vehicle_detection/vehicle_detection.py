...
class VehicleCounter(YoloLpd):
    ...
    def initialize_video_writer(
        ...
        return cv2.VideoWriter(save_path, fourcc, sourceVideofps,
                               (video_width, video_height), True)

    def box_in_previous_frames(self, previous_frame_detections, current_box,
                               current_detections):
        ...
        for i in range(self.FRAMES_BEFORE_CURRENT):
            ...
            if len(coordinate_list
                  ) == 0:  # When there are no detections in the previous frame
                continue
        ...
        current_detections[(
            centerX, centerY)] = previous_frame_detections[frame_num][coord]
        return True

    def count_vehicles(self, boxes, classIDs, vehicle_count,
                       previous_frame_detections, frame):
        list_of_vehicles = [
            "bicycle", "car", "motorbike", "bus", "truck", "train"
        ]
        current_detections = {}
        # ensure at least one detection exists
        # if len(idxs) > 0:
        ...
        for i, box in enumerate(boxes):
            ...
            if (self.LABELS[classIDs[i]] in list_of_vehicles):
                ...
                if (not self.box_in_previous_frames(previous_frame_detections,
                                            (centerX, centerY, w, h),
                                            current_detections)):
                    vehicle_count += 1
